...
@Database(entities = {Community_Scrap.class}, version =  1)
public abstract class CommunityDatabase extends RoomDatabase {
    ...
    public abstract Community_ScrapDao todoDao();

//디비객체생성 가져오기`
    public static CommunityDatabase getAppDatabase(Context context){
        if(INSTANCE == null){
            INSTANCE = Room.databaseBuilder(context, CommunityDatabase.class , "Scrap-db")
                    .build();
